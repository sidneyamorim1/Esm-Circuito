...
export function solveLinearSystem(A: number[][], B: number[]): number[] {
  ...
  for (let i = 0; i < n; i++) {
    ...
    // Se o pivô for muito pequeno, regulariza o sistema em vez de interromper a simulação.
    // Isso evita que nós flutuantes ou subcircuitos isolados derrubem a interface inteira.
    if (Math.abs(a[i][i]) < EPS) {
      let rowMagnitude = 0;
      for (let j = i; j < n; j++) {
        rowMagnitude += Math.abs(a[i][j]);
      }
...
type DiodeMode = 'off' | 'forward' | 'reverse';
...
export function runSimulationStep(
  ...
  components.forEach(comp => {
    if (comp.type === 'source_dc' || comp.type === 'bench_supply') {
    ...
    } else if (comp.type === 'ammeter' || comp.type === 'voltmeter' || comp.type === 'multimeter') {
      // Instrumentos: voltímetros são alta impedância; amperímetros fecham o ramo
      // como uma fonte de 0V para permitir a leitura da corrente no MNA.
      const multimeterMode = String(comp.properties.mode?.value ?? 'voltage');
      if (comp.type === 'ammeter' || (comp.type === 'multimeter' && multimeterMode === 'current')) {
        voltageSources.push({
          compId: comp.id,
          nodePos: terminalToNode[getTerminalKey(comp.id, 'p')],
          nodeNeg: terminalToNode[getTerminalKey(comp.id, 'n')],
          voltage: 0,
          type: comp.type === 'multimeter' ? 'multimeter_current' : 'ammeter'
        });
      }
    } else if (comp.type === 'megohmmeter') {
      const isTesting = comp.simulationState?.custom?.isTesting ?? (comp.properties.isTesting?.value === true);
      const testVoltage = Number(comp.properties.testVoltage?.value ?? 500);
      if (isTesting) {
        voltageSources.push({
          compId: comp.id,
          nodePos: terminalToNode[getTerminalKey(comp.id, 'line')],
          nodeNeg: terminalToNode[getTerminalKey(comp.id, 'earth')],
          voltage: testVoltage,
          type: 'megohmmeter'
        });
      }
      ...
      const zenerVoltage = comp.type === 'zener'
  ...
  const diodeMode: Record<string, DiodeMode> = {};
  diodes.forEach(d => {
    diodeMode[d.compId] = 'off'; // Começa cortado
  });
  ...
  while (!converged && numIterations < maxIterations) {
    ...
    // Diodos em condução
    diodes.forEach(d => {
      const mode = diodeMode[d.compId];
      if (mode === 'forward') {
        // Diodo conduzindo diretamente = fonte de tensão de queda direta (vOn)
        extraRestr.push({ compId: d.compId, n1: d.nodeAnode, n2: d.nodeCathode, vDrop: d.vOn, type: 'diode_on' });
      } else if (mode === 'reverse' && d.type === 'zener') {
        // Zener em ruptura reversa = tensão fixa do cátodo para o ânodo.
        extraRestr.push({ compId: d.compId, n1: d.nodeCathode, n2: d.nodeAnode, vDrop: d.zenerVoltage ?? 5.1, type: 'zener_reverse' });
      }
    });
    ...
    diodes.forEach(d => {
      ...
      const currentMode = diodeMode[d.compId];
      let nextMode = currentMode;

      if (currentMode === 'off' && vd > d.vOn) {
        // Tensão excedeu a queda direta, diodo entra em condução
        nextMode = 'forward';
        stateChanged = true;
      } else if (currentMode === 'off' && d.type === 'zener' && -vd > (d.zenerVoltage ?? 5.1)) {
        // Tensão reversa excedeu a tensão Zener, entra em ruptura reversa
        nextMode = 'reverse';
        stateChanged = true;
      } else if (currentMode !== 'off') {
        // Diodo conduzindo: determinamos a corrente através da restrição MNA correspondente
        // A corrente MNA é a corrente que flui no sentido da restrição ativa.
        ...
        if (restrIdx !== -1) {
          ...
          const iDiode = solutionVec[currentMnaIdx];
          if (iDiode < 0) {
  ...
  components.forEach(comp => {
    ...
    if (comp.type === 'resistor' || comp.type === 'ldr' || comp.type === 'motor_dc') {
      ...
      if (diodeMode[comp.id] !== 'off') {
        ...
        const diodeIdx = diodes.filter(d => diodeMode[d.compId] !== 'off').findIndex(d => d.compId === comp.id);
        if (diodeIdx !== -1) {
          const restrMnaIdx = numNodes + activeVoltageSources.length + activeSwCount + diodeIdx;
          const solvedCurrent = solutionVec[restrMnaIdx];
          current = diodeMode[comp.id] === 'reverse' ? -solvedCurrent : solvedCurrent;
        }
      } else {
        current = 0;
      }
      ...
      const nPos = getTermNode('p');
      const nNeg = getTermNode('n');
      const mode = String(comp.properties.mode?.value ?? 'voltage');
      if (comp.type === 'multimeter' && mode === 'current') {
        voltage = 0; // ideal
        const srcIdx = activeVoltageSources.findIndex(s => s.compId === comp.id);
        current = srcIdx !== -1 ? solutionVec[numNodes + srcIdx] : 0;
      } else {
        voltage = getNodeVoltage(nPos) - getNodeVoltage(nNeg);
      ...
      const nLine = getTermNode('line');
      const nEarth = getTermNode('earth');
      voltage = getNodeVoltage(nLine) - getNodeVoltage(nEarth);
      const isTesting = comp.simulationState?.custom?.isTesting ?? (comp.properties.isTesting?.value === true);
      const testVoltage = Number(comp.properties.testVoltage?.value ?? 500);
      ...
      const srcIdx = activeVoltageSources.findIndex(s => s.compId === comp.id);
      if (srcIdx !== -1 && isTesting) {
        current = -solutionVec[numNodes + srcIdx];
        leakCurrent = Math.abs(current);
        voltage = testVoltage;
        if (leakCurrent > 1e-12) {
          resistance = testVoltage / leakCurrent;
          if (resistance > 2e9) {
            displayRes = '> 2000 MΩ';
          } else if (resistance >= 1e6) {
            displayRes = `${(resistance / 1e6).toFixed(1)} MΩ`;
          } else if (resistance >= 1e3) {
            displayRes = `${(resistance / 1e3).toFixed(1)} kΩ`;
          } else {
            displayRes = `${resistance.toFixed(1)} Ω`;
          }
        } else {
          resistance = Infinity;
          displayRes = '> 2000 MΩ';
        ...
        current = 0;
        displayRes = isTesting ? '> 2000 MΩ' : 'OFF';
      }
  ...
  const estimateWireCurrents = () => {
    ...
    components.forEach(comp => {
      if (comp.type === 'source_dc' || comp.type === 'bench_supply' || comp.type === 'source_ac' || comp.type === 'source_pulse' || comp.type === 'function_generator' || comp.type === 'megohmmeter') {
        if (comp.type === 'megohmmeter') {
          addTwoTerminalSource(comp, 'line', 'earth');
        } else {
        ...
        comp.type === 'ammeter' ||
        (comp.type === 'multimeter' && String(comp.properties.mode?.value ?? 'voltage') === 'current')
      ) {
        addTwoTerminalLoad(comp, 'p', 'n');
      } else if (comp.type === 'transistor_bjt_npn' || comp.type === 'transistor_bjt_pnp') {
        const i = componentStates[comp.id]?.current ?? 0;
