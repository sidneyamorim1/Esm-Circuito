...
interface CircuitStore {
  ...
  updateComponentLabelRotation: (id: string, rotation: number) => void;
...
export const useStore = create<CircuitStore>((set, get) => ({
  ...
  updateComponentLabelRotation: (id, rotation) => {
    get().pushHistory();
    set((state) => ({
      components: state.components.map((c) => (c.id === id ? { ...c, labelRotation: rotation } : c)),
      project: { ...state.project, updatedAt: new Date().toISOString() }
    }));
  },
  addWire: (wire) => {
  ...
  pasteSelection: (targetGridPosition) => {
    ...
    const minX = Math.min(...clipboard.components.map(comp => comp.x));
    const minY = Math.min(...clipboard.components.map(comp => comp.y));
    const offsetX = targetGridPosition ? targetGridPosition.x - minX : 3;
    const offsetY = targetGridPosition ? targetGridPosition.y - minY : 3;

    const stripCopySuffix = (name: string) => name.replace(/\s+Copia(?:\s+\d+)?$/i, '');
    ...
    clipboard.components.forEach(comp => {
      ...
      const newComp: CircuitComponent = {
        ...
        id: newId,
        name: `${stripCopySuffix(comp.name)} Copia`,
        x: newX,
        y: newY,
