...
import { signUpUser, listUsersFromProfiles, updateUserPassword, updateUserAiAccess } from '../services/supabaseService';
...
export default function AdminModal({ isOpen, onClose }: AdminModalProps) {
  ...
  const [users, setUsers] = useState<Array<{ id: string; email: string; role: string; has_ai_access: boolean; createdAt: string }>>([]);
  ...
  const handleToggleAiAccess = async (userId: string, currentAccess: boolean) => {
    await updateUserAiAccess(userId, !currentAccess);
    fetchUsers();
  };
                    ...
                    <div key={u.id} className="p-3.5 flex items-center justify-between text-xs hover:bg-slate-900/50 transition-colors">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className={`px-2.5 py-1 rounded-full text-[10px] font-extrabold uppercase border flex items-center gap-1 transition-all ${
                            u.has_ai_access
                              ? 'bg-emerald-500/20 text-emerald-300 border-emerald-500/40 hover:bg-emerald-500/30'
                              : 'bg-slate-800 text-slate-400 border-slate-700 hover:bg-slate-700'
                          }`}
                          title={u.has_ai_access ? 'Clique para revogar acesso à IA' : 'Clique para liberar acesso à IA'}
                        >
                          {u.has_ai_access ? <Sparkles size={11} className="text-emerald-400" /> : <Lock size={11} />}
                          <span>{u.has_ai_access ? 'IA: Ativo' : 'IA: Bloqueado'}</span>
                        </button>
