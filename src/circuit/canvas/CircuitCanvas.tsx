...
export default function CircuitCanvas() {
  ...
  const getGridCoordsFromEvent = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    const worldX = (mouseX / currentViewport.zoom) - currentViewport.x;
    const worldY = (mouseY / currentViewport.zoom) - currentViewport.y;
  ...
  const getGridCoordsFromClientPoint = (clientX: number, clientY: number) => {
    const canvas = canvasRef.current;
    if (!canvas) return { x: 0, y: 0, gridX: 0, gridY: 0 };
  ...
  const isProbeComponent = (comp: CircuitComponent) => comp.type === 'probe_dc' || comp.type === 'probe_ac';
  ...
  const componentLocalToWorld = (comp: CircuitComponent, localX: number, localY: number) => {
    const sx = comp.mirrorX ? -1 : 1;
    const sy = comp.mirrorY ? -1 : 1;
    const mirroredX = localX * sx;
    const mirroredY = localY * sy;
    const angle = (comp.rotation * Math.PI) / 180;
    return {
      x: comp.x * GRID_SIZE + mirroredX * Math.cos(angle) - mirroredY * Math.sin(angle),
      y: comp.y * GRID_SIZE + mirroredX * Math.sin(angle) + mirroredY * Math.cos(angle)
    };
  };
  ...
  const getComponentLabelHit = (worldX: number, worldY: number) => {
    for (let i = components.length - 1; i >= 0; i--) {
      ...
      // As pontas de prova usam o rótulo como parte do corpo visual.
      // Evitamos "arrastar o texto" separadamente para não descolar o badge da agulha.
      if (isProbeComponent(comp)) continue;
      ...
      const height = 14;
      const labelWorld = componentLocalToWorld(comp, labelX, labelY);
      const labelAngle = -((comp.labelRotation ?? 0) * Math.PI) / 180;
      const dx = worldX - labelWorld.x;
      const dy = worldY - labelWorld.y;
      const hitX = dx * Math.cos(labelAngle) - dy * Math.sin(labelAngle);
      const hitY = dx * Math.sin(labelAngle) + dy * Math.cos(labelAngle);
  ...
  const trySelectElementAt = (worldX: number, worldY: number) => {
    ...
    for (let i = components.length - 1; i >= 0; i--) {
      ...
      if (isProbeComponent(comp)) {
        // Probe compacto: ponta + leitura numérica curta.
        // Mantemos a área de clique pequena para não “inflar” a peça no canvas.
        minX = -10;
        maxX = 28;
        minY = -22;
        maxY = 12;
  ...
  const duplicateActiveSelection = () => {
    copyActiveSelection();
    const pasted = pasteStoreSelection({ x: mouseGridPos.x, y: mouseGridPos.y });
    if (!pasted) return;
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (activeTool === 'select') {
      ...
      const selectedComp = selectedId && !isWireId(selectedId) && !selectedId.startsWith('text_')
        ? components.find(c => c.id === selectedId)
        : null;
      const clickedProbe = selectedComp ? isProbeComponent(selectedComp) : false;

      if (selectedId && !clickedProbe) {
        // Auto-wire continua valendo para componentes normais,
        // mas probes simples precisam priorizar arraste para não travar no terminal central.
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (isPanning) {
      const currentViewport = useStore.getState().viewport;
      const dx = (e.clientX - panStart.x) / currentViewport.zoom;
      const dy = (e.clientY - panStart.y) / currentViewport.zoom;
      setViewport({
    ...
    if (e.button === 0 && selectionBox) {
      ...
      if (dx > 5 || dy > 5) {
        ...
        if (canvas) {
          ...
          const currentViewport = useStore.getState().viewport;
          const wX1 = (x1 / currentViewport.zoom) - currentViewport.x;
          const wX2 = (x2 / currentViewport.zoom) - currentViewport.x;
          const wY1 = (y1 / currentViewport.zoom) - currentViewport.y;
          const wY2 = (y2 / currentViewport.zoom) - currentViewport.y;
  ...
  const handleWheel = (e: React.WheelEvent<HTMLCanvasElement>) => {
    ...
    const nextZoom = Math.max(0.4, Math.min(currentViewport.zoom * zoomFactor, 3.0));
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Rotação da peça (R/+) e rotação independente do nome (Shift+R).
      if ((e.key.toLowerCase() === 'r' || e.key === '+') && selectedComponentId) {
        e.preventDefault();
        if (e.shiftKey && e.key.toLowerCase() === 'r') {
          const comp = components.find(c => c.id === selectedComponentId);
          const nextRotation = ((((comp?.labelRotation ?? 0) + (e.altKey ? -15 : 15)) % 360) + 360) % 360;
          updateComponentLabelRotation(selectedComponentId, nextRotation);
        } else {
          const deltaRotation = e.altKey ? 180 : 90;
      ...
      if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'v') {
        e.preventDefault();
        e.stopImmediatePropagation();
        const pasteTarget = lastMouseClientPosRef.current
          ? getGridCoordsFromClientPoint(lastMouseClientPosRef.current.x, lastMouseClientPosRef.current.y)
          : { gridX: mouseGridPos.x, gridY: mouseGridPos.y };
        if (storeClipboard) {
          pasteActiveClipboard({ x: pasteTarget.gridX, y: pasteTarget.gridY });
        } else if (clipboard) {
          handlePasteAction(pasteTarget.gridX, pasteTarget.gridY);
        }
      }
        ...
        <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-sm z-[2000] flex items-center justify-center p-4">
          <div className="bg-white/95 dark:bg-slate-900/95 border border-slate-200/50 dark:border-slate-800/50 rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden flex flex-col backdrop-blur-md text-slate-800 dark:text-slate-100">
            ...
            <div className="px-5 py-4 space-y-4 max-h-[350px] overflow-y-auto">
              {components.find(c => c.id === propertiesModalCompId) && (
                <div className="flex flex-col space-y-1.5">
                  ...
                  <div className="flex items-center space-x-2">
                    <input
                      type="number"
                      value={components.find(c => c.id === propertiesModalCompId)?.labelRotation ?? 0}
                      onChange={(e) => {
                        updateComponentLabelRotation(propertiesModalCompId, Number(e.target.value) || 0);
                      }}
