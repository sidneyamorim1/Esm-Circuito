...
import { getPcbConnections, getPcbPhysicalComponents } from '../circuit/pcb/pcbNetlist';
...
export default function App() {
  ...
  const [showMountingHoles, setShowMountingHoles] = useState(true);
  const [mountingHoleDiameter, setMountingHoleDiameter] = useState(3.2);
  const [mountingHoleMargin, setMountingHoleMargin] = useState(5);
  const [showSolderPads, setShowSolderPads] = useState(true);
  const [solderPadDiameter, setSolderPadDiameter] = useState(1.6);
  const [pcbLayout, setPcbLayout] = useState<Record<string, { x: number; y: number; rotation?: number }>>({});
  const [pcbRoutes, setPcbRoutes] = useState<Record<string, { points: { x: number; y: number }[]; layer?: 'top' | 'bottom'; width?: number; color?: string; manual?: boolean }>>({});
  const pcbPhysicalComponents = React.useMemo(
    () => getPcbPhysicalComponents(components),
    [components]
  );
  const pcbPhysicalWires = React.useMemo(
  ...
  useEffect(() => {
    if (isSupabaseConfigured()) {
      getCurrentUser().then((user) => {
        if (user) {
          setAuthSession({ id: user.id, name: user.name, email: user.email, role: user.role, has_ai_access: user.has_ai_access });
        }
      });
  ...
  const [topWindow, setTopWindow] = useState<'osc' | 'fgen' | 'megger'>('osc');
  ...
  // Megômetro (Insulation Tester)
  const [meggerWindowOpen, setMeggerWindowOpen] = useState(false);
  const [meggerWindowMinimized, setMeggerWindowMinimized] = useState(false);
  const [meggerWindowPosition, setMeggerWindowPosition] = useState({ x: 220, y: 80 });
  const [meggerDragOffset, setMeggerDragOffset] = useState<{ x: number; y: number } | null>(null);
  const [meggerCompId, setMeggerCompId] = useState<string | null>(null);
  ...
  const applyBoardFromProject = (projectData: any) => {
    ...
    setShowMountingHoles(projectData.board.showMountingHoles ?? true);
    setMountingHoleDiameter(Number(projectData.board.mountingHoleDiameter) || 3.2);
    setMountingHoleMargin(Number(projectData.board.mountingHoleMargin) || 5);
    setShowSolderPads(projectData.board.showSolderPads ?? true);
    setSolderPadDiameter(Number(projectData.board.solderPadDiameter) || 1.6);
  ...
  const zoomSchematicAtCenter = (zoomFactor: number) => {
    const canvas = document.getElementById('schematic-canvas') as HTMLCanvasElement | null;
    const currentViewport = useStore.getState().viewport;
    if (!canvas) {
      setViewport({ zoom: Math.max(0.4, Math.min(currentViewport.zoom * zoomFactor, 3)) });
      return;
    }
    ...
    const worldX = (centerX / currentViewport.zoom) - currentViewport.x;
    const worldY = (centerY / currentViewport.zoom) - currentViewport.y;
    const nextZoom = Math.max(0.4, Math.min(currentViewport.zoom * zoomFactor, 3));
  ...
  useEffect(() => {
    if (isSimulating) {
      ...
      const meggerComp = comps.find(c => c.type === 'megohmmeter');
      if (meggerComp) {
        setMeggerCompId(meggerComp.id);
        setMeggerWindowOpen(true);
        setMeggerWindowMinimized(false);
      }
    }
  }, [isSimulating]);
  ...
  // Drag do painel do Megômetro
  useEffect(() => {
    if (!meggerDragOffset) return;

    const handleMove = (event: MouseEvent) => {
      setMeggerWindowPosition({
        x: Math.max(10, Math.min(window.innerWidth - 300, event.clientX - meggerDragOffset.x)),
        y: Math.max(10, Math.min(window.innerHeight - 100, event.clientY - meggerDragOffset.y))
      });
    };

    const handleUp = () => setMeggerDragOffset(null);
    ...
    <div className="flex flex-col w-full h-full overflow-hidden text-slate-800 dark:text-slate-200 transition-colors duration-200 select-none">
      ...
      <div className="flex flex-1 flex-row min-h-0 overflow-hidden bg-slate-50 dark:bg-slate-950">
            ...
            viewMode === 'schematic' ? (
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-5 text-slate-800 dark:text-slate-200 text-left">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex flex-col space-y-1.5">
                    <label className="text-[10px] font-bold text-slate-400 uppercase">Tamanho da placa</label>
                    <select
                      value={boardPreset}
                    ...
                    >
                      {BOARD_PRESETS.map(preset => (
                        <option key={preset.id} value={preset.id}>
                          {preset.id === 'custom'
                            ? `Personalizada - ${Math.round(boardDimensions.width * 10)} x ${Math.round(boardDimensions.height * 10)} mm`
                    ...
                    {boardPreset === 'custom' && (
                      <p className="text-[10px] text-slate-400 leading-normal">
                        Defina o tamanho customizado nos campos de largura e altura abaixo.
                      </p>
                    )}
                  </div>
                ...
                <div className="space-y-3">
                  <h4 className="text-xs font-bold text-indigo-650 dark:text-indigo-400 uppercase tracking-wider">Fabricação</h4>

                  <label className="flex items-center justify-between gap-3 text-xs">
                    <span className="font-bold text-slate-500">Furação da placa</span>
                    <input
                      type="checkbox"
                      checked={showMountingHoles}
                      onChange={(e) => setShowMountingHoles(e.target.checked)}
                      className="w-4 h-4"
                    />
                  </label>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="flex flex-col space-y-1.5">
                      <label className="text-[10px] font-bold text-slate-400 uppercase">Furo</label>
                      <div className="flex items-center gap-1">
                        <input
                          ...
                          max="8"
                          step="0.1"
                          value={mountingHoleDiameter}
                          onChange={(e) => setMountingHoleDiameter(Math.max(1, Number(e.target.value) || 3.2))}
                          className="w-full text-xs px-2 py-1.5 rounded-md border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 font-mono"
                        />
                        <span className="text-xs font-bold text-slate-500">mm</span>
                      </div>
                    </div>
                    <div className="flex flex-col space-y-1.5">
                      ...
                      <div className="flex items-center gap-1">
                        <input
                          ...
                          max="30"
                          step="0.5"
                          value={mountingHoleMargin}
                          onChange={(e) => setMountingHoleMargin(Math.max(2, Number(e.target.value) || 5))}
                          className="w-full text-xs px-2 py-1.5 rounded-md border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 font-mono"
                        />
                        <span className="text-xs font-bold text-slate-500">mm</span>
                      </div>
                  ...
                  <label className="flex items-center justify-between gap-3 text-xs">
                    <span className="font-bold text-slate-500">Ilhas de solda</span>
                    <input
                      type="checkbox"
                      checked={showSolderPads}
                      onChange={(e) => setShowSolderPads(e.target.checked)}
                      className="w-4 h-4"
                    />
                  </label>

                  <div className="flex flex-col space-y-1.5">
                    <label className="text-[10px] font-bold text-slate-400 uppercase">Diâmetro da ilha</label>
                    <div className="flex items-center gap-1">
                      <input
                        type="number"
                        min="0.8"
                        max="5"
                        step="0.1"
                        value={solderPadDiameter}
                        onChange={(e) => setSolderPadDiameter(Math.max(0.8, Number(e.target.value) || 1.6))}
                        className="w-full text-xs px-2 py-1.5 rounded-md border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 font-mono"
                      />
                      <span className="text-xs font-bold text-slate-500">mm</span>
                    </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="text-xs font-mono space-y-2">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-bold text-slate-700 dark:text-slate-200">{pcbPhysicalComponents.length}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-400">Conexões Físicas:</span>
                      <span className="font-bold text-slate-700 dark:text-slate-200">{pcbPhysicalWires.length}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  {(() => {
                    const isOutside = pcbPhysicalComponents.some(component => {
                      const layoutPos = pcbLayout[component.id] ?? { x: 0, y: 0 };
                      return Math.abs(layoutPos.x) > boardDimensions.width / 2 ||
                        Math.abs(layoutPos.y) > boardDimensions.height / 2;
                    });
        ...
        <main className="flex-1 flex flex-col relative min-w-0 min-h-0 overflow-hidden">
          ...
          <div className="flex-1 min-h-0 relative bg-slate-50 dark:bg-slate-950">
            {viewMode === 'schematic' ? (
              ...
              <PcbLayoutEditor
                ...
                showMountingHoles={showMountingHoles}
                mountingHoleDiameter={mountingHoleDiameter}
                mountingHoleMargin={mountingHoleMargin}
                showSolderPads={showSolderPads}
                solderPadDiameter={solderPadDiameter}
                layout={pcbLayout}
          ...
          {/* Painel Flutuante do Megômetro */}
          {meggerWindowOpen && (() => {
            const meggerComp = components.find(c => c.id === meggerCompId || c.type === 'megohmmeter');
            const isTesting = meggerComp?.simulationState?.custom?.isTesting ?? (meggerComp?.properties.isTesting?.value === true);
            const testVoltage = Number(meggerComp?.properties.testVoltage?.value ?? 500);
            const displayRes = meggerComp?.simulationState?.custom?.displayRes ?? (isTesting ? 'MEDINDO' : 'OFF');
            const leakCurrent = Number(meggerComp?.simulationState?.custom?.leakCurrent ?? 0);
            const realVoltage = Number(meggerComp?.simulationState?.custom?.voltage ?? 0);

            const toggleTesting = () => {
              if (meggerComp) {
                useStore.getState().updateComponentProperty(meggerComp.id, 'isTesting', !isTesting);
              }
            };

            const setTestVoltage = (v: number) => {
              if (meggerComp) {
                useStore.getState().updateComponentProperty(meggerComp.id, 'testVoltage', v);
              }
            };

            const formatCurrent = (i: number) => {
              if (i === 0) return '0.00 nA';
            ...
            return (
              <div
                onMouseDown={() => setTopWindow('megger')}
                style={{
                  left: meggerWindowPosition.x,
                  top: meggerWindowPosition.y,
                  zIndex: topWindow === 'megger' ? 99999 : 99988,
                  ...(meggerWindowMinimized
                    ? {
                        width: '320px',
                        height: '42px',
                        minHeight: '42px',
                ...
                className="fixed bg-[#eab308] dark:bg-amber-950/90 border-2 border-[#a16207] dark:border-amber-700 rounded-2xl shadow-2xl flex flex-col font-sans backdrop-blur-md select-none"
              >
                {/* Header da Janela */}
                <div
                  onMouseDown={(e) => {
                    setMeggerDragOffset({
                      x: e.clientX - meggerWindowPosition.x,
                      y: e.clientY - meggerWindowPosition.y
                    });
                  }}
                  className="flex items-center justify-between px-3.5 py-2 bg-[#ca8a04] dark:bg-amber-900/90 border-b border-[#a16207] cursor-move text-amber-950 dark:text-amber-100"
                >
                  <div className="flex items-center space-x-2 font-black text-xs uppercase tracking-wider">
                    <Zap size={15} className="text-red-600 dark:text-red-400 animate-pulse" />
                    <span>MEGÔMETRO - ISOLAÇÃO</span>
                    {isTesting && (
                      <span className="text-[9px] px-1.5 py-0.5 bg-red-600 text-white rounded font-bold animate-pulse">
                        HV ON!
                      </span>
                    )}
                  </div>
                  <div className="flex items-center space-x-1">
                    <button
                      onMouseDown={e => e.stopPropagation()}
                      onClick={() => setMeggerWindowMinimized(prev => !prev)}
                      className="p-1 hover:bg-amber-500/30 rounded text-amber-950 dark:text-amber-100"
                      title={meggerWindowMinimized ? 'Restaurar' : 'Minimizar'}
                    >
                      {meggerWindowMinimized ? <Maximize size={13} /> : <Minus size={13} />}
                    </button>
                    <button
                      onMouseDown={e => e.stopPropagation()}
                      onClick={() => setMeggerWindowOpen(false)}
                      className="p-1 hover:bg-amber-500/30 rounded text-amber-950 dark:text-amber-100"
                      title="Fechar"
                    >
                      <X size={13} />
                ...
                {!meggerWindowMinimized && (
                  <div className="p-4 space-y-4 bg-slate-900 text-slate-100">
                    {/* Display LCD Retroiluminado estilo Fluke */}
                    <div className="bg-[#022c22] border-2 border-[#059669] rounded-xl p-3 shadow-inner relative overflow-hidden">
                      <div className="flex justify-between items-center text-[10px] font-mono text-emerald-400/80 mb-1">
                        <span className="font-bold flex items-center gap-1">
                          <Activity size={12} className={isTesting ? 'text-emerald-400 animate-pulse' : 'text-slate-600'} />
                          {isTesting ? `INSULATION TEST (${testVoltage}V DC)` : 'STANDBY'}
                        </span>
                        <span className={`px-1.5 py-0.5 rounded font-black text-[9px] ${isTesting ? 'bg-red-500/20 text-red-400 border border-red-500/40 animate-pulse' : 'bg-slate-800 text-slate-400'}`}>
                          {isTesting ? 'HIGH VOLTAGE' : 'OFF'}
                        </span>
                      </div>

                      {/* Leitura Principal MΩ / GΩ */}
                      <div className="text-center my-2">
                        <div className="text-[10px] uppercase font-mono tracking-widest text-emerald-500/70">Resistência de Isolação</div>
                        <div className="text-3xl font-black font-mono text-emerald-400 tracking-wider">
                          {displayRes}
                        </div>
                      </div>

                      {/* Telemetria Auxiliar (Tensão Real + Corrente de Fuga) */}
                      <div className="grid grid-cols-2 gap-2 pt-2 border-t border-emerald-900/60 font-mono text-xs">
                        <div className="bg-[#064e3b]/40 p-1.5 rounded border border-emerald-800/40">
                          <div className="text-[9px] text-emerald-400/70">Tensão no Teste</div>
                          <div className="font-bold text-emerald-300">{isTesting ? `${realVoltage.toFixed(0)} V DC` : '0 V'}</div>
                        </div>
                        <div className="bg-[#064e3b]/40 p-1.5 rounded border border-emerald-800/40 text-right">
                          <div className="text-[9px] text-emerald-400/70">Corrente de Fuga</div>
                          <div className="font-bold text-emerald-300">{isTesting ? formatCurrent(leakCurrent) : '0.00 nA'}</div>
                        </div>
                      </div>
                    </div>

                    {/* Seleção de Tensão de Teste */}
                    <div>
                      <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5 block">
                        Tensão de Teste DC (Test Voltage)
                      </label>
                      <div className="grid grid-cols-4 gap-1.5">
                    ...
                    {/* Botão Principal PRESS TO TEST */}
                    <div className="pt-1">
                      <button
                        onMouseDown={e => e.stopPropagation()}
                        onClick={toggleTesting}
                        className={`w-full py-3.5 rounded-xl font-black text-sm uppercase tracking-wider flex items-center justify-center space-x-2 transition-all shadow-lg active:scale-95 ${
                          isTesting
                            ? 'bg-red-600 hover:bg-red-500 text-white shadow-red-600/30 animate-pulse border-2 border-red-400'
                            : 'bg-gradient-to-r from-amber-500 to-orange-600 hover:from-amber-400 hover:to-orange-500 text-slate-950 shadow-amber-500/20 border-2 border-amber-400'
                        }`}
                      >
                        <Zap size={18} fill={isTesting ? 'white' : 'currentColor'} />
                        <span>{isTesting ? 'PARAR TESTE (TEST STOP)' : 'INICIAR TESTE (PRESS TO TEST)'}</span>
                      </button>
                    </div>
                  </div>
                )}
                ...
                <div>
                  ...
                  <div className="mb-4">
                    ...
                    <div className="flex items-center space-x-2 mt-1">
                      <input
                        type="number"
                        value={selectedComponent.labelRotation ?? 0}
                        onChange={(e) => {
                          useStore.getState().updateComponentLabelRotation(selectedComponent.id, Number(e.target.value) || 0);
                        }}
                      ...
                      />
                      <span className="text-xs font-mono font-bold text-slate-500 w-8">°</span>
                    </div>
                  </div>
      ...
      <ProjectsHubModal
        ...
        onNewProject={() => {
          ...
          setShowMountingHoles(true);
          setMountingHoleDiameter(3.2);
          setMountingHoleMargin(5);
          setShowSolderPads(true);
          setSolderPadDiameter(1.6);
          setPcbLayout({});
          setPcbRoutes({});
        }}
